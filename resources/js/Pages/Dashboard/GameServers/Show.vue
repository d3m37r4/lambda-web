...
const deletePunishmentReason = (punishmentReason) => {
    deleteMethod(route('dashboard.game-servers.punishment-reasons.destroy', [
            props.gameServer, punishmentReason
        ]),
        {
            preserveState: true,
            preserveScroll: true,
...
const deleteMethod = (url, { preserveState = false, preserveScroll = false } = {}) => {
    router.delete(url, {
        onBefore: (visit) => {
...
<template>
<!--    <button class="btn" @click="my_modal_3.showModal()">open modal</button>-->
    <dialog id="my_modal_3" class="modal">
        <div class="modal-box">
            <form method="dialog">
                <button @click="isDelete=true" class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>
            </form>
    ...
    <dialog id="my_modal_3" class="modal" ref="modal">
        <div class="modal-box">
            <form method="dialog">
                <button @click="isDelete = false" class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>
            </form>
            <h3 class="text-lg font-bold">Are you sure you want to delete?</h3>
            <p class="py-4">Press ESC key or click on ✕ button to cancel</p>
            <div class="modal-action">
                <button @click="confirmDelete" class="btn btn-error">Delete</button>
                <button @click="isDelete = false" class="btn">Cancel</button>
            </div>
        </div>
    </dialog>
        ...
        <div id="tab-reasons" v-show="isActive('tab-reasons')" class="">
            <div class="pb-4 flex justify-end">
                <CreateButton
                    title="Добавить"
                    class="btn-sm"
                    :link="route('dashboard.game-servers.punishment-reasons.create', gameServer)" />
            </div>
                        ...
                        <tr v-for="(punishmentReason, index) in punishmentReasons" class="hover:bg-base-300">
                            ...
                            <td>
                                ...
                                <div class="lg:tooltip" data-tip="Удалить">
                                    <button @click="deletePunishmentReason(punishmentReason)" class="btn btn-sm btn-circle btn-outline btn-error">
                                        <svg class="h-5 w-5" viewBox="0 0 48 48" fill="currentColor"><path d="M12 38c0 2.21 1.79 4 4 4h16c2.21 0 4-1.79 4-4v-24h-24v24zm26-30h-7l-2-2h-10l-2 2h-7v4h28v-4z"/><path d="M0 0h48v48h-48z" fill="none"/></svg>
                                    </button>
                                </div>
                            </td>
